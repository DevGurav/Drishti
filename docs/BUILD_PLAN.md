...
| DEC-025 | Medicine mode takes the **earliest** of multiple expiry dates on a strip | A real Paracip strip carries two dates (`EXP.OCT.2026` carton, `EXP.APR.28` blister). Taking whichever OCR line came first reported APR.28 — 18 months later than the true expiry, entirely by accident of OCR line ordering. The medicine cannot be trusted past the earliest date, so `parsers.earliest_expiry()` picks the minimum of every parseable candidate |
| DEC-026 | PaddlePaddle/PyTorch collision (`DEC-006`) also breaks OCR + IndicTransToolkit, not just OCR + the VLM | Empirically reproduced running notebook 04 on Colab: OCR then translation in one kernel triggered `AsyncIOLoopKernelRestarter`, no traceback. `KMP_DUPLICATE_LIB_OK` reduces frequency, does not eliminate it. Notebook 04 is now split into a PaddlePaddle-only phase and a PyTorch-only phase with one mandatory restart between them, bridged by a JSON checkpoint on disk (session restart clears Python state but not the VM's disk) |
