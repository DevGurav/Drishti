...
| Currency mode | ✅ Trained, ⚠️ **numbers restated** | Colab T4 2026-08-10, 12 epochs, 4002 images / 8 classes. Headline was 0.9883 / ₹5.37, but 33.5% of that test split leaks from train (`DEC-049`). **On the 399 clean images: accuracy 0.9875, expected error ₹7.52; at `CONFIDENCE_THRESHOLD=0.90` it answers 82.5% at 0.9939 and ₹1.09.** Quote those. Checkpoint 6.2 MB. Misses ≥99% unconditionally — clears it only by declining. Validated on notes, not arbitrary scenes (`DEC-042`) |
