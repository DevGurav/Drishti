...
| Currency mode | ✅ **Retrained on the merged corpus** | Colab T4 2026-08-11, 12 epochs, **5,602 images / 8 classes, split verified 0.0% leaked**. Accuracy **0.9810**, expected error **₹10.87**; at `CONFIDENCE_THRESHOLD=0.90` it answers **89.4%** at **0.9947** and **₹3.57**. The threshold sweep re-derived 0.90 independently on clean data. `DEC-041`'s systematic ₹20→₹200 and ₹50→₹500 confusions are **gone**; ₹2000 errors now carry 86% of all rupee cost (`DEC-051`). **All five real handheld fixtures now answer, against 3 of 5 before** — `curr-200` 0.342→0.926 (`DEC-052`). Checkpoint 6.2 MB |
