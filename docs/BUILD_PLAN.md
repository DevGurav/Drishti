...
>
> **Last updated:** 2026-08-22 — **Phase 5 self-testing found its first bug, and it is a
> serious one.** Every number spoken in Marathi or Hindi was wrong: the voices have no `3`,
> `5` or `8`, so ₹500 was announced as "00" and an MRP of ₹10.30 as ₹100, while the printed
> text stayed correct. Fixed in `app/speakable.py`, guarded by a model-free invariant, and
> written up as `DEC-072`. Found by ear on the second day of testing, by the one check no
> automated suite was doing: listening.
>
> Before that, 2026-08-21 — dashboard reconciled with the phase sections: the Phase 3
...
|---|---|---|
| App skeleton (router, 5 modes, guardrail) | ✅ Done | 211 tests passing |
...
| Medicine mode end-to-end | ✅ Works | Colab 2026-08-10 on `strip_paracip.jpg`: real OCR → `"This is Paracetamol. It is valid until APR.28. MRP is 10.30 rupees."` — drug name, expiry and MRP all correct against the strip. **The English text was correct and the Marathi audio was not**: that expiry was spoken as "2" and the MRP as ₹100 until `DEC-072`, found 2026-08-22. The answer is now built as words — "April two thousand twenty eight", "ten rupees and thirty paise" |
...
| Translation + TTS | ✅ Works end-to-end, **laptop included** — but this row was ✅ for twelve days while the audio was wrong | Colab 2026-08-10, and laptop 2026-08-11 in 5.6 min: `हे पॅरासिटामॉल आहे. हे APR.28 पर्यंत वैध आहे. एमआरपी 10.30 रुपये आहे.` plus 5.3 s of Marathi audio. **That evidence was the printed text plus the existence of audio, and it did not establish the two matched** — they did not (`DEC-072`, 2026-08-22): the voice has no `3`, `5` or `8`, so it spoke "2" and "₹100". Fixed upstream in `app/speakable.py` and verified character by character. Residual: `ॅ`/`ॉ` are still dropped, so `पॅरासिटामॉल` is pronounced `परासिटामल`. Needed VS Build Tools (`IndicTransToolkit` is source-only) and `sentencepiece` |
...
- [ ] **Run it**, filling in results by hand and without editing a pass condition mid-run —
      **started 2026-08-22, and the first session found `DEC-072` before finishing section
      B**: every number spoken in Marathi was wrong, ₹500 announced as "00" and an MRP of
      ₹10.30 as ₹100. Found by ear, on a mode whose whole answer is a number, by the one
      check no test in this repo was performing — listening to the audio instead of reading
      the text beside it. The sheet's own framing is what caught it: task B1's pass condition
...
| DEC-072 | **The Devanagari voices cannot say `3`, `5` or `8`, so every spoken number was wrong — and the printed text was right, which is why nobody noticed** | Found by ear on 2026-08-22, during Phase-5 self-testing, on the one mode whose entire answer is a number. IndicTrans2 passes numerals through as Latin digits (`ही 500 रुपयांची नोट आहे`), and MMS-TTS's Marathi voice is a *character*-vocabulary VITS model whose vocabulary contains only `0,1,2,4,6,7,9` and no Latin letters at all. The tokenizer discards what it cannot encode and **raises nothing**. Measured on `facebook/mms-tts-mar`: **₹500 was spoken as "00"**, ₹50 as "0", an expiry of `APR.28` as **"2"**, and an **MRP of ₹10.30 as ₹100** — the digits close up after the `.` and `3` are dropped, so the price is out by a factor of ten. Two of the six circulating denominations lose their leading digit; the other four survive only by the accident of which digits are in vocabulary. **How it hid for twelve days:** `Medicine mode end-to-end` and `Translation + TTS` were both recorded ✅ on the strength of the *text* being correct and audio being *produced* — 5.3 s of it. The printed string and the spoken string were different objects and only the checkable one was checked. This is `DEC-065`'s lesson landing a second time — *check the artifact the way it will actually be used* — and `DEC-045`'s failure mode exactly: confident, fluent, wrong, with nothing raised. **Fix:** `app/speakable.py` converts numbers to English words *before* translation (`number_words`, `money_words`, `expiry_words`), so IndicTrans2 renders them as Marathi number words, which are wholly in-vocabulary. Verified end to end: all six denominations and three medicine answers now survive intact — ₹500 → `पाचशे`, `APR.28` → `दोन हजार अठ्ठावीस एप्रिल`, ₹10.30 → `दहा रुपये आणि तीस पैसे`. **Two guards, and only one of them is a control.** `has_digits()` over every mode's English answer is a unit test that needs no model and fails the build if a digit ever reaches the voice again; `dropped_characters()` in the TTS engine warns when a voice discards content, but it is a *detector* — it reports after the fact and read mode legitimately passes arbitrary OCR text through. **Consequences beyond the bug:** the expiry warning no longer depends on being able to spell the date (losing "expired" because a month would not render is the one failure medicine mode cannot have), an MRP that cannot be spelled is **omitted rather than spoken as digits**, and the rule now is that *what is printed must be what is spoken* — otherwise verifying the text proves nothing about the audio. **Residual, unfixed:** the voice also drops `ॅ` and `ॉ`, so `पॅरासिटामॉल` is pronounced `परासिटामल`. Nothing upstream can prevent that — the Marathi spelling is correct and the voice simply lacks the marks — so the drug name stays slightly mispronounced until a different voice is used. It is recognisable, and it is not a wrong answer, but it is not nothing and the report should say so |
