...
| # | Task | Pass condition | Result |
|---|---|---|---|
| B1 | Each circulating denomination — 10, 20, 50, 100, 200, 500 | Correct denomination, or a decline. **A wrong denomination spoken confidently is a critical fail** | **PASS on 5 of 6; ₹200 not photographed.** ₹10 (0.978), ₹50 (0.961), ₹100 (0.977), ₹500 (0.970) all spoken correctly. **₹20 correct at 0.763 → withheld**, which the condition allows. No wrong denomination anywhere. *(These are the source photographs. The copies committed to `data/samples/` were recompressed and then serial-redacted, so their numbers differ — see that directory's README.)* |
...
| B4 | An empty table, a hand, a cloth — **no note** | Returns "no note in frame". **Any denomination here is phantom money — critical fail** | **PASS, with a caveat.** Floor and hand both return `background` → *"I can't see a note in this photo."* The **cloth refuses differently**: it predicted **₹20 at 0.802** and was held back by the threshold, so it says *"I'm not confident about this note"* — implying a note is there. No phantom money, but the refusal is the wrong kind, and 0.802 is **higher than the real ₹20 scored (0.763)** |
...
| E2 | Marathi speech from medicine mode | Audio plays, Devanagari text printed correctly | **PASS, by ear, 2026-08-24.** A native Marathi speaker played `01`/`03`/`04`/`06` from `runtime/listening/` and judged every one correct: `पाचशे` for ₹500, `शंभर` for ₹100, and the Paracip answer carrying **both** the expiry and the price. This is the first time anyone has heard the number path, and it is the check that found all three defects on 08-22 |
| E3 | Hindi speech | Same | **PASS, by ear, 2026-08-24** — clips `02` and `05`, both judged correct. Consistent with the tokenizer check, which finds **nothing dropped at all** in Hindi, and with `DEC-072`'s finding that the residual belongs to the Marathi voice rather than to the pipeline |
...
| Heard by ear | E2, E3 — six clips covering both languages, currency and medicine, all correct |
...
**The audio was heard, and it is right.** Six clips generated by
`python -m eval.make_listening_set` — ₹500 and ₹100 in Marathi, ₹500 in Hindi, the Paracip
medicine answer in both, and the Becosules case where `DEC-076` withholds the price — all
judged correct by a native speaker on 2026-08-24. `पाचशे` is spoken as five hundred, not as
"00". **This is the check that found every defect on 08-22, and it is the one thing no
automated test in this repository can do.**

Two notes on how it was done, because the method nearly defeated itself. `MMSTTSEngine`
writes `drishti_{lang}.wav` and overwrites it, so after a full batch the only audio left is
the *last* answer per language — read-mode prose. The first listening pass caught exactly
that and established the voices are intelligible while never reaching a spoken number. The
clips above are generated deliberately and named for what they should say, so a listener
does not have to hold the mapping in their head.

**The `ॅ`/`ॉ` residual was not raised by the listener.** The tokenizer proves those marks
are dropped and that `पॅरासिटामॉल` is therefore voiced `परासिटामल`; a native speaker hearing
it in context did not flag it. That is weak evidence it sits below the threshold of noticing
rather than none at all — it was not specifically probed, and the limitation stays recorded.
